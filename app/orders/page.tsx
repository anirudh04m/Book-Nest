                ...
                {order.items && order.items.length > 0 ? (
                  <ul className="space-y-2">
                    {order.items.map((item, index) => {
                      ...
                      return (
                        <li key={item.order_item_id || index} className="flex justify-between items-center text-gray-700">
                          <div className="flex-1">
                            <span className="font-medium">{item.description || `Item #${item.item_id || 'N/A'}`}</span>
